    ...
    def minimaxAlphaBeta(self, state, player, depth=5, frm=(0, 0), to=(0, 0), alpha = -99, beta = 99):
        ...
        if player == "hare":
            ...
            for move in possibleMoves:
                nextState = copy.deepcopy(state)
                nextState.movePosition(move[0], move[1])
                tempScore, tempFrm, tempTo = self.minimax(nextState, "wolf", depth - 1, move[0], move[1], alpha, beta)
            ...
            for move in possibleMoves:
                nextState = copy.deepcopy(state)
                nextState.movePosition(move[0], move[1])
                tempScore, tempFrm, tempTo = self.minimax(nextState, "hare", depth - 1, move[0], move[1],alpha, beta)
